...
void send_packet(int p_id,void *p)
{
    ...
    //cout << "Packet [" << p_type << "]\n";
    EX_OVER *s_over=new EX_OVER; //로컬변수 금지 send 재사용
    s_over->m_op = OP_SEND;
    memset(&s_over->m_over, 0, sizeof(s_over->m_over));
    memcpy(s_over->m_packetbuf, p, p_size);
    s_over->m_wsabuf[0].buf = reinterpret_cast<CHAR *>(s_over->m_packetbuf);
    s_over->m_wsabuf[0].len = p_size;
    short ret = WSASend(players[p_id].m_socket, s_over->m_wsabuf, 1, NULL, 0,&s_over->m_over, 0);
    if (0 != ret) {
...
void do_recv(int s_id)
{
    ...
    DWORD r_flag = 0;
    short ret = WSARecv(players[s_id].m_socket, players[s_id].m_recv_over.m_wsabuf, 1, NULL, &r_flag, &players[s_id].m_recv_over.m_over, NULL);
    /*if (0 != ret) {
        auto err_no = WSAGetLastError();
        if (WSA_IO_PENDING != err_no)
